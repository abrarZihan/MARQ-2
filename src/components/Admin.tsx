...
import { Building2, CheckCircle2, Clock, Trash2, Edit2, UserPlus, FileText, Pickaxe, HardHat, Package, Users, Zap, Droplets, Paintbrush, Box, CircleDollarSign, XCircle, RefreshCw, UserMinus, Building, ShieldPlus, ShieldMinus, KeyRound, Key, ClipboardList, Shield, Search, X } from "lucide-react";
    ...
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 dark:border-slate-800 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className="text-sm font-extrabold text-slate-900 dark:text-slate-100">{log.adminName}</span>
          <span className={cn("px-2 py-0.5 rounded-md text-[10px] font-bold", m.bg, m.color)}>{label}</span>
          {prj && <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">{prj.name}</span>}
        </div>
        ...
        <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 font-medium">{fmtTs(log.ts)}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pb-20">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-black text-slate-900 dark:text-slate-100">{t('common.activity_log')}</h1>
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{t('common.records_count', { count: filtered.length })}</p>
        </div>
        {isSuperAdmin && logs.length > 0 && (
          <button 
            onClick={() => setShowClearConfirm(true)}
            className="flex items-center gap-2 px-3 py-2 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-xl text-xs font-bold hover:bg-rose-100 dark:hover:bg-rose-800 transition-colors"
          >
            <Trash2 size={14} />
            {t('common.clear_logs')}
          </button>
      ...
      <div className="relative mb-3 group">
        <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-slate-600 dark:group-focus-within:text-slate-300 transition-colors" />
        <input 
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-2">
        {filtered.length === 0 ? (
          <div className="text-center py-10 text-slate-400 dark:text-slate-500 font-medium">{t('common.no_records')}</div>
        ) : (
